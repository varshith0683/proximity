...
RTSP_URL = "rtsp://admin:123456@192.168.1.6:554/unicast/c2/s1/live"
...
class PersonDistanceDetector:
    def __init__(self, rtsp_url, output_dir="output"):
        ...
        if not ret:
            raise RuntimeError("RTSP stream is not delivering frames")
        ...
        print("[INFO] Camera FPS:", self.measured_fps)
        ...
        try:
            while not self.stop:
                ...
                for i, box in enumerate(boxes):
                    ...
                    cv2.putText(frame, f"{dist:.1f}m", (x1, y1 - 8),
                                cv2.FONT_HERSHEY_SIMPLEX, 0.6, color, 2)

                    if crossed:
                        print(f"[ALERT] Person crossed {crossed}m at {elapsed:.2f}s")
                        self.alerts.append({
    ...
    def save_evidence(self):
        ...
        print("[INFO] Evidence saved:", path)
...
if __name__ == "__main__":
    detector = PersonDistanceDetector(RTSP_URL, "output")
    detector.run()
