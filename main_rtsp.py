...
class PersonDistanceDetector:
    def __init__(self, output_dir="output"):
        ...
        if not self.cap.isOpened():
            raise RuntimeError("[ERROR] Failed to connect to RTSP camera")

        print("[CAMERA] RTSP camera connected")
        ...
        # ---------------- RECORDING ----------------
        # self.recording_path = os.path.join(output_dir, "rtsp_recording.mp4")
    ...
    def run(self):
        ...
        try:
            while not self.stop:
                ...
                if not ret:
                    print("[ERROR] RTSP frame read failed")
                    break
...
if __name__ == "__main__":
    detector = PersonDistanceDetector(output_dir="output")
    detector.run()
